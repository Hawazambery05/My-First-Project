...
if(isset($_POST['student_id']))
{
	...
	$query = mysqli_query($con, "SELECT * FROM student WHERE student_id='$user' AND password='$pass'");
	$row = mysqli_fetch_assoc($query);
		
	//message if failed
	if(mysqli_num_rows($query) == 0||$row['password'] !=$pass ||$row['verification_code'] != 1)
	{
		echo "<script> alert('You have not verified your email');
		window.location = 'verify-student.php'</script>";
	}
	else if(mysqli_num_rows($query) == 0||$row['password'] !=$pass)
	{
